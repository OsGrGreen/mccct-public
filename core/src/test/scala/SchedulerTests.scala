...
class SchedulerTests {
  ...
  /**
    * A test that tests if all futures are executed when using the RandomWalk algorithm
    * In this test there are no awaits, however all futures should still be executed, and completed before the main thread can continue
    */
  @Test
  ...
  /**
    * A test that makes sure that all futures are executed when using the RandomWalk algorithm
    * This test has nested futures and multiple awaits.
    * This allows us to say some things about the execution, like if we await for the top-level task, then its child task has to be completed before we can continue (if the parent task waits for the child task)
    */
  @Test
  ...
  /**
    * A test that makes sure that the reset function works as expected.
    * This means that all previous information is cleaned with reset
    */
  @Test
  def resetTest(): Unit = {
  ...
  /**
    * A test that makes sure that if a top-level futures await is hit before all top-level futures have been started then:
  ...
  def awaitTest(): Unit = {
    ...
    assert(appearsAfter("3.", "1.", schedule)) // Since f3 is only executed after f1 has finished then that should also be reflected in the schedulers schedule
  ...
  /**
    * A test that makes sure that tasks are not executed before it is needed
    * In this case this means that all top-level tasks have to be started before execution is allowed
    */
  @Test
  ...
  /** A function that determines if an element occurs before or after another element
    ...
    * @param list, the list in which the elements appears
