...
import org.apache.pekko.dispatch.{PriorityGenerator, UnboundedPriorityMailbox}
...
/** @author
  ...
  */
object NQueensPekkoPriorityActorBenchmark {

  @main def NQueensPriority(args: String*) = {
    BenchmarkRunner.runBenchmark(args.toArray, new NQueensPekkoPriorityActorBenchmark)
  }

  private final class NQueensPekkoPriorityActorBenchmark extends Benchmark {
    def initialize(args: Array[String]) = {
      PekkoActorState.setPriorityMailboxType(classOf[NQueensPekkoPriorityMailbox].getCanonicalName)
    ...
    def runIteration() = {
      ...
      master(0) =
        system.actorOf(Props(new Master(numWorkers, priorities)).withDispatcher("Pekko.actor.prio-dispatcher"))
  ...
  private class Master(numWorkers: Int, priorities: Int) extends PekkoActor[AnyRef] {
    ...
    override def onPostStart() = {
      var i: Int = 0
      while (i < numWorkers) {
        workers(i) = context.system.actorOf(Props(new Worker(self, i)).withDispatcher("Pekko.actor.prio-dispatcher"))
...
class NQueensPekkoPriorityMailbox(settings: ActorSystem.Settings, config: Config)
    extends UnboundedPriorityMailbox(
      // Create a new PriorityGenerator, lower priority means more important
      PriorityGenerator {
        case wm: NQueensConfig.WorkMessage => wm.priority
        case wm: NQueensConfig.StopMessage => 0
        case PoisonPill                    => 100
        case otherwise                     => 100
      }
    )
