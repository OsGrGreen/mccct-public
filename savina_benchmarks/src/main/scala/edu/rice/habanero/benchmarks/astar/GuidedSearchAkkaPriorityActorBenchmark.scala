...
/** @author
  ...
  */
object GuidedSearchPekkoPriorityActorBenchmark {

  @main def GuidedSearchPriority(args: String*) = {
    BenchmarkRunner.runBenchmark(args.toArray, new GuidedSearchPekkoPriorityActorBenchmark)
  }

  private final class GuidedSearchPekkoPriorityActorBenchmark extends Benchmark {
    def initialize(args: Array[String]) = {
      PekkoActorState.setPriorityMailboxType(classOf[GuidedSearchPekkoPriorityMailbox].getCanonicalName)
    ...
    def runIteration() = {
      ...
      val master = system.actorOf(Props(new Master()).withDispatcher("Pekko.actor.prio-dispatcher"))
...
class GuidedSearchPekkoPriorityMailbox(settings: ActorSystem.Settings, config: Config)
