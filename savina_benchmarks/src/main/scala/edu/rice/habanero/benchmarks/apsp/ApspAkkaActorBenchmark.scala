...
/** @author
  ...
  */
object ApspPekkoActorBenchmark {

  @main def Apsp(args: String*) = {
    BenchmarkRunner.runBenchmark(args.toArray, new ApspPekkoActorBenchmark)
  }

  private final class ApspPekkoActorBenchmark extends Benchmark {
    ...
    def runIteration() = {
      ...
      val system = PekkoActorState.newActorSystem("ForkJoin")

      // create and automatically the actors
      val blockActors = Array.tabulate[ActorRef](numBlocksInSingleDim, numBlocksInSingleDim) { (i, j) =>
        ...
        PekkoActorState.startActor(apspActor)
        apspActor
      }
  ...
  private class ApspFloydWarshallActor(
      myBlockId: Int,
      blockSize: Int,
      graphSize: Int,
      initGraphData: Array[Array[Long]]
    ...
    override def process(msg: AnyRef) = {
      msg match {
        ...
        case ApspNeighborMessage(msgNeighbors) =>
          ...
          msgNeighbors.foreach { loopNeighbor =>
            neighbors.append(loopNeighbor)
          }
      }
    }
    ...
    private def notifyNeighbors(): Unit = {
      ...
      neighbors.foreach { loopNeighbor =>
        sendTo(resultMessage, loopNeighbor)
      }
    }
  }
