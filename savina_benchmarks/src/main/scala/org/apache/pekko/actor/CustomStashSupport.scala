...
import org.apache.pekko.dispatch.{DequeBasedMessageQueueSemantics, Envelope, Mailboxes, RequiresMessageQueue}
...
trait CustomUnrestrictedStash extends Actor with CustomStashSupport {
  ...
  /** Overridden callback. Prepends all messages in the stash to the mailbox, clears the stash, stops all children and
    * invokes the postStop() callback.
    */
  ...
  /** Overridden callback. Prepends all messages in the stash to the mailbox and clears the stash. Must be called when
    * overriding this method, otherwise stashed messages won't be propagated to DeadLetters when actor stops.
    */
...
/** INTERNAL API.
  *
  * Support trait for implementing a stash for an actor instance. A default stash per actor (= user stash) is maintained
  * by [[UnrestrictedStash]] by extending this trait. Actors that explicitly need other stashes (optionally in addition
  * to and isolated from the user stash) can create new stashes via [[StashFactory]].
  */
private[pekko] trait CustomStashSupport {
    ...
    actorCell.mailbox.messageQueue match {
      ...
      case other                                  ⇒
        throw ActorInitializationException(
          ...
          |  my-custom-mailbox {
          |    mailbox-type = "Pekko.dispatch.UnboundedDequeBasedMailbox"
          |  }
          | """.stripMargin
        )
    }
  ...
  /** Prepends the oldest message in the stash to the mailbox, and then removes that message from the stash.
    *
    * Messages from the stash are enqueued to the mailbox until the capacity of the mailbox (if any) has been reached.
  ...
  /** Enqueues `envelope` at the first position in the mailbox. If the message contained in the envelope is a
    * `Terminated` message, it will be ensured that it can be re-received by the actor.
    */
